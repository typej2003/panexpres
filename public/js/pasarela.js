        ...
        Array.from(forms).forEach(form => {
            form.addEventListener('submit', async  event => {
                ...
                if (form.checkValidity()) {
                    let username = document.getElementById('cellphone').value;
                    let profile = document.getElementById('profile').value;
                    let sessionTimeout = document.getElementById('sessionTimeout').value;
                    console.log('Operacion previa');
                    let operacion = await createCuenta(username, profile, sessionTimeout);
                    console.log('resultado de createCuenta:', operacion);
                    console.log(operacion);
                    if(operacion.data.data.result.code == 'USER_CREATE'){                         
                        createPayment();
                    }else{
                        console.log('Error al crear el usuario');
                        showMessagePasarela('Error al crear la cuenta. ' + operacion.data.data.result.message, 'danger');
                        if(operacion.data.data.result.message=='El usuario ya existe en el hotspot'){
                            document.getElementById('btnCreatePayment').style.display = 'none';
                            document.getElementById('renew account').style.display = 'block';
                        }
                        return false;
                        // messageAreaPasarela('Error al crear la cuenta. Error:' + operacion.message, 'danger');
                    }                    
                }
                else {
